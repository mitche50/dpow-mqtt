...
def init():
    ...
    if r.get("bpow_last_day") is None:
        r.set("bpow_last_day", datetime.now().day)
    if r.get("bpow_last_hour") is None:
        r.set("bpow_last_hour", datetime.now().hour)
    if r.get("bpow_last_minute") is None:
        r.set("bpow_last_minute", datetime.now().minute)
...
if __name__ == "__main__":
    ...
    hour_difference = int(r.get("bpow_last_hour")) - datetime.now().hour
    if hour_difference >= 1:
        ...
        avg_data = db.get_avg()
        populate_chart(avg_data, "avg")
    day_difference = int(r.get("bpow_last_day")) - datetime.now().day
    if day_difference >= 1:
        print("Day difference greater than 1, setting data")
        day_data = db.get_day_list()
        populate_chart(day_data, "day")
    minute_difference = int(r.get("bpow_last_minute")) - datetime.now().minute
